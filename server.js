...
app.post('/api/upload', upload.single('file'), async (req, res) => {
    ...
    const videoId = generateRandomId(); // Generate a random ID for the video
    const fileExtension = path.extname(req.file.originalname);
    const filename = `${videoId}${fileExtension}`; // Store file with unique ID
    ...
    blobStream.on('finish', () => {
        const publicUrl = `https://storage.googleapis.com/${bucketName}/${filename}`;
        res.status(200).json({ id: videoId, url: publicUrl }); // Return the file URL
    });
